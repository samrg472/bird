  ...
  program.addHelpText(
    ...
    () =>
      `\n${ctx.colors.section('Examples')}\n${[
        ...
        `  Add ${ctx.colors.option('--json')} to: read, replies, thread, search, mentions, bookmarks, likes, following, followers, about, lists, list-timeline, user-tweets, query-ids, dms, dm, profile`,
      )}\n${ctx.colors.muted(
